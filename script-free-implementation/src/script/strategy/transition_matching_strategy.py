...
from script.operation import (CodableOperation, LocatableOperation, AssertElement, ExecuteScript,
                              PageTransition, Step)
...
class TransitionMatchingStrategy(Strategy):
    def __init__(self, model):
        self.__driver_manager = DriverManager(Setting.APP_SPECIFIC_CHROME_OPTIONS)
...
""".format(
    ...
    def __get_page_variations(self, steps_in_page: List[LocatableOperation]) -> List[PageVariation]:
        ...
        execute_script_variations = self.__get_variation_by_type(
            steps_in_page, vector_calculator, OperationType.EXECUTE_SCRIPT
        )[: Setting.SEARCH_WIDTH]
